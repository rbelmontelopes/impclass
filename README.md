...
- There is also a option to use a manually downloaded set of genomes (*--manual_dataset*) to allow more control of the genomes used for the core database. For this you can go to the NCBI Genomes page (https://www.ncbi.nlm.nih.gov/datasets/genome/), search for the taxon of interest (i.e. *Ascomycota*), filter to see only reference genomes, select the taxa you want by selecting them in the boxes at the left side, and then clicking in Download > Download Package, and in the pop-up window selecting only Genome Sequences (FASTA) (there is no need for any other files appart from the Assembly data report that is always downloaded by default). Is possible to pass the zip file downloaded from the NCBI directly, but if already extracted, pass the path that goes until the *ncbi_dataset* folder.
...
The first time the pipeline is run, it will create the conda enviroments that are needed to run all the steps, what could take some time. The pipeline is enabled to use mamba instead of conda to be faster, but you need to have mamba installed, otherwise will fallback to using conda to create the environments. If you already have all the dependencies installed, an option is to change in the modules *download_or_load.nf* the lines *conda "${baseDir}/envs/ncbi.yml"*, in the module *busco.nf* the lines *conda "${baseDir}/envs/busco.yml"*, and in the module *orthofinder.nf* the lines *conda "${baseDir}/envs/orthofinder.yml"* to the respective paths were the conda enviroments are (i.e. ~/anaconda/envs/orthofinder) or simply remove these lines if everything is already reachable from the base terminal.
